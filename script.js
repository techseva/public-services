...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateAdminState() {
        ...
        if (fabButton) { // fabButton is 'addBtn'
        ...
        // Change Title to Admin Panel
        const appBarTitle = document.querySelector('.app-bar .app-title');
        if (appBarTitle) {
            appBarTitle.innerText = isAdmin ? "SPS Admin Panel" : "Smart Public Services";
        }
    ...
    // Admin Secret: Click "Updates" tab 10 times
    const allTabs = document.querySelectorAll('.tab');
    allTabs.forEach(tab => {
        if (tab.innerText.trim().toUpperCase() === 'UPDATES') {
            tab.addEventListener('click', (e) => {
                // If they are on index.html, the first click would navigate.
                // We let it navigate, but if they are already on updates.html,
                // they can click 10 times. Or we prevent navigation while they are rapidly clicking.
                ...
                if (clickCount >= 3) {
                    ...
                    if (clickCount >= 5 && clickCount < 10) {
                        showToast(`Secret: ${10 - clickCount} clicks left...`);
                    }
                }
