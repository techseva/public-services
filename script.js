...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Admin Secret: Click "Updates" tab 10 times (Replaces title-click method)
    document.querySelectorAll('.tab').forEach(tab => {
        if (tab.innerText.trim().toUpperCase() === 'UPDATES') {
            tab.addEventListener('click', (e) => {
                ...
                dbTimer = setTimeout(() => { clickCount = 0; }, 2000);

                if (clickCount >= 3) {
                    e.preventDefault();
                    if (clickCount >= 5 && clickCount < 10) showToast(`Admin Mode: ${10 - clickCount} more...`);
                }
