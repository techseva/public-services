...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateAdminState() {
        ...
        if (syncBtn) {
            syncBtn.style.display = isAdmin ? 'flex' : 'none';
        }
    }
    ...
    if (syncBtn) {
        syncBtn.addEventListener('click', () => {
            if (confirm("Push all your local buttons/pages to the Cloud for everyone?")) {
                syncLocalToCloud();
            }
        });
    }
...
function syncLocalToCloud() {
    if (!isOnline) return alert("Firebase not connected. Cannot sync.");
    ...
    alert("🎉 Sync Complete! " + total + " items pushed to Cloud. Everyone can see them now!");
    location.reload();
}
