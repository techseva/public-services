...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize Firebase
    // Check if we have a valid key (either from localstorage or hardcoded)
    ...
    function updateAdminState() {
        ...
        if (fabButton) {
            if (isAdmin) fabButton.classList.add('visible');
            else fabButton.classList.remove('visible');
        }

        if (adminToggleBtn) {
            if (isAdmin) {
                adminToggleBtn.style.display = 'flex';
                const textNode = adminToggleBtn.childNodes[adminToggleBtn.childNodes.length - 1];
                if (textNode) textNode.textContent = "Disable Admin Mode";
            } else {
    ...
    titleElements.forEach(title => {
        title.addEventListener('click', () => {
            ...
            dbTimer = setTimeout(() => { clickCount = 0; }, 1000);

            if (clickCount === 10) {
                const currentStatus = localStorage.getItem('appAdminMode') === 'true';
                const newStatus = !currentStatus;
                localStorage.setItem('appAdminMode', newStatus);
                updateAdminState();
                alert(newStatus ? "Admin Mode Enabled!" : "Admin Mode Disabled");
                clickCount = 0;
            }
        });
    });
    ...
    function openMenu() { if (sideMenu) { sideMenu.classList.add('active'); menuOverlay.classList.add('active'); } }
    function closeMenu() { if (sideMenu) { sideMenu.classList.remove('active'); menuOverlay.classList.remove('active'); } }
    ...
    if (addBtn) {
        addBtn.addEventListener('click', () => {
            ...
            if (pageContext === 'main') {
                ...
                if (inpActionType) { inpActionType.value = 'page'; inpActionType.closest('.form-group').style.display = 'none'; }
    ...
    window.openEditModal = function (id, type) {
        ...
        if (type === 'static') {
            alert("Core system services cannot be edited. You can only remove them.");
            return;
        }
    ...
    function loadUpdates() {
        if (isOnline) {
            db.ref('updates').on('value', (snapshot) => {
                ...
                const updates = data ? Object.values(data) : [];
                // Sort by ID/Date descending if possible, need array sort
                renderUpdatesUI(updates.reverse());
            });
        } else {
    ...
    function renderUpdatesUI(updates) {
        ...
        // Clear dynamic items only (keep static if any? No, list is container)
        // Wait, index.html/updates.html has static items inside. We should clear them if we want pure sync?
        // The user wants 'fix functionality', usually means full dynamic.
        // But to respect previous static items, we must clear them OR we assume the DB is the source of truth.
        // Let's clear the container to avoid duplicates.
        list.innerHTML = '';
    ...
    function renderServicesUI(services, contextId) {
        ...
        if (!list) return;
        list.innerHTML = ''; // Requesting full sync view
        ...
        services.forEach(service => {
            ...
            if (service.type === 'page') {
                div.onclick = function (e) {
                    ...
                    window.location.href = `page.html?id=${service.pageId}&title=${encodeURIComponent(service.title)}`;
                };
            } else {
                div.onclick = function (e) {
                    if (e.target.closest('.card-menu-btn') || e.target.closest('.card-menu-dropdown')) return;
    ...
    function handleSaveAction(context) {
        ...
        if (context === 'updates') {
            ...
            let actionType = 'page';
            if (context !== 'main') actionType = 'link';
    ...
    function handleDeleteAction(id, type, context) {
        ...
        if (type === 'static') {
            ...
            location.reload(); // Static requires reload usually
            return;
        }
...
function enhanceStaticCards() {
    const hiddenItems = JSON.parse(localStorage.getItem('hiddenStaticItems')) || [];
    const cards = document.querySelectorAll('.service-card, .update-card');
    cards.forEach(card => {
        if (card.innerHTML.includes('card-menu-btn')) return;
        const titleEl = card.querySelector('h3') || card.querySelector('.card-title');
        if (!titleEl) return;
        const subEl = card.querySelector('.card-subtitle') || card.querySelector('.update-body');
        const id = 'static_' + titleEl.innerText.trim().replace(/[^a-zA-Z0-9]/g, '');

        // Add to cache so Edit works (Read-only though)
        // ...

        if (hiddenItems.includes(id)) { card.style.display = 'none'; }
        else {
            ...
            if (originalClick) card.onclick = function (e) { if (e.target.closest('.card-menu-btn') || e.target.closest('.card-menu-dropdown')) return; originalClick.call(this, e); };
        }
    });
}
