    ...
    document.querySelectorAll('.tab').forEach(tab => {
        if (tab.innerText.trim().toUpperCase() === 'UPDATES') {
            tab.addEventListener('click', (e) => {
                ...
                // Navigation Fixing: 
                // 1. If we are on index.html, the VERY FIRST click must go to updates.html
                // 2. If we are already on updates.html, we prevent navigation to allow secret clicking
                ...
                if (isOnUpdatesPage) {
                    ...
                    if (clickCount >= 2) {
                        e.preventDefault(); // Prevent navigation after the first intended click
                    } else {
                        // Let the first click navigate naturally
                        // Optional: window.location.href = 'updates.html';
                    }
                }
